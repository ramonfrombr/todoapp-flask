...
from todoapp.apis.v1 import api_v1
from todoapp.blueprints.auth import auth_bp
from todoapp.blueprints.home import home_bp
from todoapp.blueprints.todo import todo_bp
from todoapp.extensions import db, login_manager, csrf, babel
from todoapp.models import User, Item
from todoapp.settings import config
...
def register_commands(app):
    ...
    def init(locale):
        ...
        if os.system(
                'pybabel init -i messages.pot -d todoapp/translations -l ' + locale):
    ...
    def update():
        ...
        if os.system('pybabel extract -F babel.cfg -k _l -o messages.pot .'):
            raise RuntimeError('extract command failed')
        if os.system('pybabel update -i messages.pot -d todoapp/translations'):
    ...
    def compile():
        ...
        if os.system('pybabel compile -d todoapp/translations'):
