...
from todoapp.apis.v1.auth import auth_required, generate_token
from todoapp.apis.v1.errors import api_abort, ValidationError
from todoapp.apis.v1.schemas import user_schema, item_schema, items_schema
...
class ItemsAPI(MethodView):
    ...
    def get(self):
        ...
        per_page = current_app.config['todoapp_ITEM_PER_PAGE']
...
class ActiveItemsAPI(MethodView):
    ...
    def get(self):
        ...
        pagination = Item.query.with_parent(g.current_user).filter_by(done=False).paginate(
            page, per_page=current_app.config['todoapp_ITEM_PER_PAGE'])
        items = pagination.items
        current = url_for('.items', page=page, _external=True)
        prev = None
        if pagination.has_prev:
